...
%     stop_criterion=sqrt(norm(derivative_Y_1, 'fro')^2+norm(derivative_Y_2, 2)^2)/d_norm;
    stop_criterion=norm(derivative_Y_1, 'fro')^2/d_norm;
    if stop_criterion<tol || inner_round >max_iter
        break;
    end
%     if mod(inner_round, display_period)==0
%         disp(['        inner round ', num2str(inner_round), ' stop_criterion=',num2str(stop_criterion), 'rank(A)=',num2str(rank(A)), '||E||_1=',num2str(sum(sum(abs(E)))), 'deta_tau', num2str(delta_tau)]);
%     end
